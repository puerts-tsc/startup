...
namespace Runtime
{
    public static partial class Strings
    {
        public static string Join<T>( this IEnumerable<T> list, string str = ", " ) =>
            string.Join( str, list.Select( t => $"{t}" ) );
        ...
        public static string ToPadSides( this string str, int totalWidth = -1, char paddingChar = '-' )
        {
            ...
            return $" {str} ".ToCapitalStr().PadLeft( padLeft, paddingChar ).PadRight( totalWidth, paddingChar )
                .ToBold();
        }

        public static string ToCapitalStr( this string str ) =>
            Regex.Replace( str, @"\b[a-z]", m => m.Value.ToUpper() );

        static int PadLength = 40;

        //
        static string DebugStr( this object str, params object[] extra ) =>
            ( $"{str} " + string.Join( " | ",
                extra.Select( t =>
                    t is Type type ? type.GetNiceFullName() :
                    t is string s ? s :
        ...
        public static string Md5( this string str )
        {
            using ( var hash = MD5.Create() ) {
                return string.Concat( hash.ComputeHash( Encoding.UTF8.GetBytes( str ) )

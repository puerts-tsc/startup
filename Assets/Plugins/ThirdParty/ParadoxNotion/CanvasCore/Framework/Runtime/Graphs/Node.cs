...
namespace NodeCanvas.Framework
{
    ...
    /*abstract*/ public partial class Node : IGraphElement, ISerializationCollectable
    {
        ...
        {
            // fix: 编辑器模式下删除子图时删除绑定的GameObject
            if ( !Application.isPlaying ) {
                if ( this is FlowNestedFlow flow ) {
                    ( flow.targetAgent.serializedValue as Component )?.gameObject?.DestroySelf();
                }
                else if ( this is FlowNestedBT bt ) {
                    ( bt.targetAgent.serializedValue as Component )?.gameObject?.DestroySelf();
                }
                else if ( this is FlowNestedFSM fsm ) {
                    ( fsm.targetAgent.serializedValue as Component )?.gameObject?.DestroySelf();
                }
                else if ( this is FlowNestedDT flowNestedDT ) {
                    ( flowNestedDT.targetAgent.serializedValue as Component )?.gameObject?.DestroySelf();
                }
                else if ( this is NestedFSM nestedFsm ) {
                    nestedFsm.subNodeAgent.value?.gameObject?.DestroySelf();
                }
                else if ( this is SubTree subTree ) {
                    subTree.subNodeAgent.value?.gameObject?.DestroySelf();
                }
                else if ( this is BTNestedFlowScript btNestedFlowScript ) {
                    btNestedFlowScript.subNodeAgent.value?.gameObject?.DestroySelf();
                }
                else if ( this is NestedDT nestedDT ) {
                    nestedDT.subNodeAgent.value?.gameObject?.DestroySelf();
                }
                else if ( this is FlowScriptState flowScriptState ) {
                    flowScriptState._nodeAgent.value?.gameObject?.DestroySelf();
                }
                else if ( this is NestedBTState nbt ) {
                    nbt._nodeAgent.value?.gameObject?.DestroySelf();
                }
                else if ( this is NestedFSMState nfsm ) {
                    nfsm._nodeAgent.value?.gameObject?.DestroySelf();
                }
                else if ( this is NestedDTState nestedDTState ) {
                    nestedDTState._nodeAgent.value?.gameObject?.DestroySelf();
                }
            }
        }

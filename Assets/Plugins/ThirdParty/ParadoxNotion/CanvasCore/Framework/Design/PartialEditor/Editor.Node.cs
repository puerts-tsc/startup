            ...
            if ( owner.graph == null ) {
                if ( asBound ) {
                    ...
                    if ( newGraph != null ) {
                        ...
                        //owner.graph.agent = go.transform;
                    }
...
//        }
    ...
    partial class Node
    {
        ...
        public bool isSelected {
            get {
                return GraphEditorUtility.activeElement == this || GraphEditorUtility.activeElements.Contains( this );
            }
        }
        ...
        public Color nodeColor {
            ...
            private set {
                if ( customColor != value ) {
                    ...
                    hexColor = ( temp.r.ToString( "X2" ) + temp.g.ToString( "X2" ) + temp.b.ToString( "X2" ) )
                        .ToLower();
                }
            }
        }
                    ...
                    if ( assignable.subGraph == null ) {
                        ...
                        if ( !Application.isPlaying && GUILayout.Button( "CREATE NEW #0x02" ) ) {
                            if ( !useAssetForSubGraph ) {
                                var root = node.graph.agent.gameObject
                                    .FindOrCreate( "GraphRoot" ); //.FindOrCreateRoot("GraphRoot");
                                var parent = root //.FindOrCreate( node.graph.agent.name)
                                    //.FindOrCreate(assignable.subGraphParameter.varType.Name)
                                    .FindOrCreate( assignable.subGraphParameter.varType.Name.Replace( "Behaviour", "" )
                                            .Replace( "Script", "" ) + "_" + node.ID +
                                        node.UID.Replace( "-", "" ).Substring( 0, 6 ) );
                                if ( assignable.subGraphParameter.varType == typeof(FlowScript) ) {
                                    assignable.subGraph = parent.CreateGraphOwner<FlowScriptController>( true, false )
                                        .graph;
                                }
                                ...
                                //assignable.graph.agent = parent.transform;
                                if ( node is FlowNestedFlow flow ) {
                                    flow.targetAgent.serializedValue = parent.transform;
                                }
                                else if ( node is FlowNestedBT bt ) {
                                    bt.targetAgent.serializedValue = parent.transform;
                                }
                                else if ( node is FlowNestedFSM fsm ) {
                                    fsm.targetAgent.serializedValue = parent.transform;
                                }
                                else if ( node is FlowNestedDT flowNestedDT ) {
                                    flowNestedDT.targetAgent.serializedValue = parent.transform;
                                }
                                else if ( node is NestedFSM nestedFsm ) {
                                    nestedFsm.subNodeAgent.value = parent.transform;
                                }
                                else if ( node is SubTree subTree ) {
                                    subTree.subNodeAgent.value = parent.transform;
                                }
                                else if ( node is BTNestedFlowScript btNestedFlowScript ) {
                                    btNestedFlowScript.subNodeAgent.value = parent.transform;
                                }
                                else if ( node is NestedDT nestedDT ) {
                                    nestedDT.subNodeAgent.value = parent.transform;
                                }
                                else if ( node is FlowScriptState flowScriptState ) {
                                    flowScriptState._nodeAgent.value = parent.transform;
                                }
                                else if ( node is NestedBTState nbt ) {
                                    nbt._nodeAgent.value = parent.transform;
                                }
                                else if ( node is NestedFSMState nfsm ) {
                                    nfsm._nodeAgent.value = parent.transform;
                                }
                                else if ( node is NestedDTState nestedDTState ) {
                                    nestedDTState._nodeAgent.value = parent.transform;
                                }

                                //assignable.subGraph.agent = parent.transform;
                                //assignable.graph.agent = parent.transform;

                                //node.graphAgent = parent.transform;
                            }
                            else {
                                ...
                                var newGraph =
                                    AssetDatabase.LoadAssetAtPath( path,
                                        assignable.subGraphParameter.varType ) as Graph;
                                if ( newGraph == null ) {
            ...
            float zoomFactor )
        {
                    ...
                    for ( var i = 0; i < outConnections.Count + 1; i++ ) {
                        ...
                        if ( GraphEditorUtility.allowClick ) {
                            ...
                            if ( e.type == EventType.MouseDown && e.button == 0 &&
                                portRect.Contains( e.mousePosition ) ) {
                                dragDropMisses = 0;
            ...
            for ( var i = 0; i < outConnections.Count; i++ ) {
                ...
                if ( connection != null ) {
                    ...
                    var sourcePos =
                        new Vector2( ( ( rect.width / ( outConnections.Count + 1 ) ) * ( i + 1 ) ) + rect.xMin,
                            rect.yMax + 6 );

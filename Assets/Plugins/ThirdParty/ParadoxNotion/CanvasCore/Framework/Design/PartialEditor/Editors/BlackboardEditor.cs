...
namespace NodeCanvas.Editor
{
    ...
    public class BlackboardEditor : EditorObjectWrapper<IBlackboard>
    {
        ...
        void InspectorGUI(IBlackboard bb, UnityEngine.Object overrideContextObject = null)
        {
            ...
            if (!bb.unityContextObject.GetAssetPath().IsNullOrEmpty()) {
                if (bb.tempName.IsNullOrEmpty()) {
                    bb.tempName = Path.GetFileNameWithoutExtension(bb.unityContextObject.GetAssetPath());
                }
                GUILayout.BeginHorizontal();
                {
                    if (GUILayout.Button("改名", GUILayout.Width(45))) {
                        if (!bb.tempName.IsNullOrWhitespace()) {
                            AssetDatabase.RenameAsset(bb.unityContextObject.GetAssetPath(), bb.tempName.Trim());

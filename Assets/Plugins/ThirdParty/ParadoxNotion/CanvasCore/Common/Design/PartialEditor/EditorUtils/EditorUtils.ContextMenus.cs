...
namespace ParadoxNotion.Design
{
	...
	partial class EditorUtils
    {
        ...
        public static MenuItemInfo[] GetMenuItems(GenericMenu menu)
        {
            ...
            if ( itemField != null ) {
                items = new ArrayList(itemField.GetValue( menu ) as ICollection );
            }
            else {
                var itemProp = typeof(GenericMenu).GetProperty("menuItems", BindingFlags.Instance | BindingFlags.NonPublic);
                if ( itemProp != null ) {
                    items = new ArrayList(itemProp.GetValue( menu ) as ICollection) ;
                }
            }

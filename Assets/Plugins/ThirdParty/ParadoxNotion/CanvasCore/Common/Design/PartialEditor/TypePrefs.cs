...
namespace ParadoxNotion.Design
{
    ...
    public static class TypePrefs
    {
        ...
        [MenuItem( "Debug/TypePrefs" )]
        static void TestFindTypePrefs()
        {
            //var jsonFile = SYNC_FILE_NAME + ".json";
            var syncFile = AssetDatabase.FindAssets( SYNC_FILE_NAME + " t:TextAsset" )
                .Select( t =>   AssetDatabase.GUIDToAssetPath( t ) ).FirstOrDefault();
            Debug.Log( syncFile );
        }
        ...
        public static string SyncFilePath()
        {
            ...
            //var jsonFile = SYNC_FILE_NAME + ".json";
            var path = AssetDatabase.FindAssets( SYNC_FILE_NAME + " t:TextAsset" )
                .Select( AssetDatabase.GUIDToAssetPath ).FirstOrDefault();
            var syncFile = AssetDatabase.LoadAssetAtPath<TextAsset>( path );

            //var syncFile = EditorGUIUtility.Load(SYNC_FILE_NAME);
            var absPath = EditorUtils.AssetToSystemPath(syncFile);
            if ( !string.IsNullOrEmpty(absPath) ) {
                Debug.Log( $"TypePrefs: {AssetDatabase.GetAssetPath( syncFile )}" );
                return absPath;
            }
            Debug.Log( $"TypePrefs: Not Found" );

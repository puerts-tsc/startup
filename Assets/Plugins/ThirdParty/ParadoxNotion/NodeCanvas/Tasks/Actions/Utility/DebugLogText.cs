        ...
        public override void OnExecute() {
           this.JsCall(nameof(OnExecute)); 
            if ( verboseMode == VerboseMode.LogAndDisplayLabel || verboseMode == VerboseMode.LogOnly ) {
                var label = $"(<b>{agent.gameObject.name}</b>) <color=#{ColorUtility.ToHtmlStringRGB(color)}>{log.value}</color>";
                if ( logMode == LogMode.Log ) {
                    ParadoxNotion.Services.Logger.Log(label, LogTag.EXECUTION, this.agent.gameObject);
                }
                if ( logMode == LogMode.Warning ) {
                    ParadoxNotion.Services.Logger.LogWarning(label, LogTag.EXECUTION, this.agent.gameObject);
                }
                if ( logMode == LogMode.Error ) {
                    ParadoxNotion.Services.Logger.LogError(label, LogTag.EXECUTION, this.agent.gameObject);
                }
            }

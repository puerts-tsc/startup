...
and run `npm i` in NodeTSCAndHotReload/js~ directory
    ...
    static string tsRoot = Path
        .GetFullPath( $"{Application.dataPath}/../{AssetDatabase.GetAssetPath( TsConfig.instance.TsRoot )}" )
        .Replace( "\\", "/" ) + "/";

    static string tsConfig = File.ReadAllText( $"{tsRoot}tsconfig.json".Trim().Replace( "\"", "'" ) );
    ...
    static void Watch()
    {
        ...
        bool result = env.Eval<bool>( @"
            ...
            try {
                const moduleRequire = require('module').createRequire('" + tsRoot + @"')
                moduleRequire('ts-node').register(" + tsConfig + @")
                global.HotReloadWatcher = moduleRequire('./src/hotReload/watch.ts').default

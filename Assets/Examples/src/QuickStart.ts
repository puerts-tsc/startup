...
import { PuertsTest, Runtime, System, Tetris, UnityEngine } from 'csharp';
...
class ClassA<T extends TestA> {
    public func<T1 extends TestA>(t: T1): void {
    }
}

global.$log = function(msg: string) {
    console.log('[test 0x1010]', msg);
}
...
export default function() {
    ...
    // let btn = new GameObject("test").GetComponent($typeof(Button))as Button;
    // btn.gameObject.name;
    ...
    JsMain.instance.gameObject.LogName();
    
    global.$log(`hello, puerts`);

//静态函数
    UnityEngine.Debug.Log('hello world:' + JsMain.instance.gameObject.name);
...
// typescript和c#的async，await联动，为了不在低版本的Unity下报错，先注释，c#7.3以上版本可以打开这些注释
    async function asyncCall() {
        ...
        let task2 = obj.GetFileLength('not_existed_file');//这个会抛文件找不到异常，被catch

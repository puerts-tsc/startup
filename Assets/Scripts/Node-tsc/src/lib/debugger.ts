...
export class Debugger extends EventEmitter {
    ...
    public async open(host: string, port?: number, local?: boolean) {
        console.log('open1');
        if (this._state === State.Connecting || this._state === State.Open) throw new Error('socket is opening');
    ...
    public update(filepath: string) {
        if (this._state !== State.Open || !this._debugger) throw new Error('socket is close');
        
        if (this._trace) {
            console.log(`change: ${ filepath }`);
        }
        if (!this._scriptParsed) return false;
        
        filepath = path.normalize(filepath).replace(/\\/g, '/');
        if (this._ignorePathCase) filepath = filepath.toLowerCase();
        return this._pushUpdate(filepath);
    }
    ...
    private async _pushUpdate(filepath: string) {
        ...
        if (scriptId && fs.existsSync(filepath) && fs.lstatSync(filepath).isFile()) {
            ...
            if (!exist || exist.scriptSource === scriptSource || !this._debugger) return false;
    ...
    private _scriptParsedHanlder = (params: ScriptParsedEvent) => {
        ...
        if (!this._scriptParsed) this._scriptParsed = {};
        this._scriptParsed[filepath] = scriptId;
        this._scriptParsed[scriptId] = filepath;
        
        if (this._checkOnStartup) this._pushUpdate(filepath);
    };
    private _scriptFailedToParseHandler = (params: ScriptFailedToParseEvent) => {
        ...
        if (!this._scriptFailedToParse) this._scriptFailedToParse = {};

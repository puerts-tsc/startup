...
namespace UnityTetris
{
    ...
    public class Playfield
    {
        ...
        public void Step()
        {
            ...
            if (IsPossibleMovement(mCurrentTetrimino.currentPosition.x, mCurrentTetrimino.currentPosition.y + 1,
                mCurrentTetrimino, mCurrentTetrimino.currentRotation)) {
        ...
        //Places 1s wherever needed whena piece either reaches bottom,
        //either collides in a way that is not possible to go lower anymore
        private void PlaceTetrimino(Tetrimino tetrimino)
        {
            for (int i1 = tetrimino.currentPosition.x, i2 = 0;
                i1 < tetrimino.currentPosition.x + Tetrimino.BLOCK_AREA;
                i1++, i2++) {
                for (int j1 = tetrimino.currentPosition.y, j2 = 0;
                    j1 < tetrimino.currentPosition.y + Tetrimino.BLOCK_AREA;
                    j1++, j2++) {
        ...
        public bool IsPossibleMovement(int x, int y, Tetrimino tetrimino, int rotation)
        {
            for (int i1 = x, i2 = 0; i1 < x + Tetrimino.BLOCK_AREA; i1++, i2++) {
                for (int j1 = y, j2 = 0; j1 < y + Tetrimino.BLOCK_AREA; j1++, j2++) {
                    ...
                    if (j1 >= 0) {
                        if (( tetrimino.ValidBlock(rotation, j2, i2) ) && ( !IsFreeBlock(i1, j1) ))
                            return false;
                    }
                }
            }

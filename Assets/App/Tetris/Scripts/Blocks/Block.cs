...
{
    public class Block : Tetris<Block> /*,Saro.IPoolable<Block>*/
    {
        ...
        private bool ValidChild()
        {
            ...
            {
                var x = Utils.Float2Int(child.position.x);
                var y = Utils.Float2Int(child.position.y);
        ...
        private bool ValidChild(Vector2 move)
        {
            foreach (Transform child in transform)
            {
                var x = Utils.Float2Int(child.position.x + move.x);
                var y = Utils.Float2Int(child.position.y + move.y);
                if (!Valid(x, y)) return false;
            }
            return true;
        }
        ...
        {
            if (x >= Gameplay.Width || x < 0 || y >= Gameplay.Height + Gameplay.ExtraHeight || y < 0) return false;
            if (Gameplay.Grid[x, y] != null) return false;
